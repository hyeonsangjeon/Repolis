...
| **Plaza Direct Link** | `?view=plaza&lang=en` | Keeps a covered 1.2-second transition, then starts the existing English owner town at its plaza without the intro. |
...
| **Atelier Direct Link** | `?repo=<owner>/<repo>&view=atelier&lang=en` | Keeps a covered 1.2-second initialization transition, then opens the exact English Repository Atelier without an intro, exterior-town, or auto-open chat step. |
...
The Plaza Direct Link accepts only fixed `view=plaza` and optional `lang=en|ko`; omitted language defaults to
English. It reuses the normal owner-town start action after the same reduced-motion-safe covered transition,
adding no request, storage, scene, or alternate spawn logic.
...
Atelier Direct Links use the stricter `?repo=owner/repo&view=atelier&lang=en` shape; `lang=ko` is also valid,
and an omitted language defaults to English. Once the target and scene are ready, the loading cover becomes a
reduced-motion-safe portal for 1.2 seconds and invokes the existing entry path with automatic chat disabled.
The cover stays up until the interior transition owns the frame, so neither the intro nor exterior town flashes.
Extra query keys, duplicate keys, an invalid language, a hash, or a mismatched target disable automatic entry
and leave the ordinary Portal confirmation intact. The direct link adds no loader, GitHub request, storage,
scene resource, or model path of its own.
...
request. Its explicit Load action reuses Repository Blueprint's one request, validates the returned
projection's owner/repo, and restores exact DOM/3D focus only when the path exists. Missing or unavailable
paths remain unselected and never trigger guessing, crawling, or cross-repository substitution.
