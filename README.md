...
- 📘 **Knowledge NPCs you can talk to** — beyond the taxi, the town has NPCs ("scholars") each grounded by **one live MCP knowledge source**. Walk up to the **MS Docs engineer** in the plaza and ask about Azure, .NET or Copilot — it searches the **official Microsoft Learn docs in real time** through an Azure AI Search **Foundry MCP Knowledge Source**, and `gpt-5.4-mini` writes the answer back **in your own language** with a trace panel linking every doc. The chat keeps **multi‑turn context**, so follow‑ups stay on topic. Every scholar is registered in **[`SCHOLARS.md`](SCHOLARS.md)** — add your own with one script.
