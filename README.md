...
- **Every repo house opens into one finished exhibition.** Its card leads to the reusable **Repository Atelier**, now carrying the current Repolis chibi, a History/Data Wall, Impact/Signals Wall, metric artifacts, a curved data path, Repository Core, and action terminals that rebind deterministically to that repo. On entry, the existing GitHub MCP explains that exact public `owner/repo`; **Ask Gitber** continues the isolated in-room thread for up to five backend calls per visit, while **Why this district?** stays local. Closing the panel preserves the thread, exiting resets it, and no other repository is substituted when evidence is missing. GitHub remains an explicit external action.
