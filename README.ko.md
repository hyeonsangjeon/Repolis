...
- **도시는 실제 기여로 이어집니다.** **오픈소스 퀘스트**는 방문자가 직접 요청할 때만 익명 GitHub 공개 검색을 한 번 실행해 현재 `good first issue`, `help wanted`, 일반 열린 이슈 중 최대 세 개를 고릅니다. 실제 레포 집을 거친 뒤 정확한 이슈로 연결하며, 결과는 탭에만 남습니다. 로그인·토큰·이슈 추적·새 백엔드는 없고, 기존 Star 초대는 방문자가 이슈 열기를 선택한 뒤에만 나타날 수 있습니다.
