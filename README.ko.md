...
- **모든 레포 집에는 완성된 전시실이 있습니다.** 카드에서 재사용 가능한 **Repository Atelier**로 입장하면 현재 Repolis 치비 캐릭터, 역사·데이터 벽, 영향·신호 벽, 지표 아티팩트, 곡선 데이터 경로, Repository Core, 액션 터미널이 선택한 레포에 맞춰 결정적으로 바뀝니다. 입장하면 기존 GitHub MCP가 정확한 공개 `owner/repo`를 자동 설명하고, **깃버에게 이 레포 질문**은 방문당 최대 다섯 번의 백엔드 호출 안에서 완전히 분리된 실내 대화를 이어갑니다. 패널만 닫으면 문맥이 유지되고 퇴장하면 초기화되며, 근거가 없을 때 다른 레포로 대신 답하지 않습니다. **왜 이 구역인가**는 로컬에서 동작하고 GitHub는 명시적인 외부 액션입니다.
