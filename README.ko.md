...
*어떤* 주제든 — `reasoning ratio` 같은 단어 하나라도 — 입력하면 세 **동료** 현자가 SSE로 **라이브 토론**을 벌여요: 🌿 **코드짱**(*옹호가*), 📜 **올드독**(*회의가*), 🌀 **썰풀이**(*분석가*)가 라운드를 거치며 **서로의 이름을 부르며** 반응·반박하고, **사람이 읽는 속도로 페이싱**돼 실제로 따라갈 수 있어요. 그다음 **시간의 의장**이 전체 기록을 읽고 판정합니다. 라이브 토론은 내부적으로 **Azure AI Foundry**로 돌아가요 — 세 현자와 판정 보조는 `gpt-5.4-mini`, 최종 선고를 내리는 의장은 더 강한 `gpt-5.4`(+reasoning)예요. 모델은 [`council/council.config.json`](council/council.config.json)에서 **하드코딩 없이 config 교체만으로** 바꿀 수 있어요. 자유 주제 판정은 **AI 추론**이라 항상 **⚡ 미검증** 배지를 달고 — 큐레이트된 6개 사례는 **결정적** 수학 판정을 유지해요. *멀티 에이전트 토론 → 심판* 패턴의 작고 정직한 한 사례입니다.
