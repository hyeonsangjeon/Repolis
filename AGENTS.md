...
| **`assets/repo-portal.js`** | Pure username/repository parser, route precedence, public projection, canonical Plaza/Portal/Atelier/Blueprint links, and owner-town expansion link. | Changing `?repo=`, `?view=plaza&lang=en`, `?view=atelier&lang=en`, accepted GitHub inputs, public traffic truth, or target/path share links. |
...
| **`cloudflare-taxi/src/repository-atelier-grounding.js`** | MCP-first identity proof, bounded public README/license reads, optional existing GitHub credential reuse with mandatory fresh public metadata, and Entra-authenticated synthesis. | Changing Atelier document answers; preserve rejected/private-reference refusal, exact-host credentials, no redirects, the shared 25-second Worker deadline, and unchanged models/budgets. |
...
| Change Plaza/Repo Portal parsing, target-first loading, direct entry, cache, expansion, or privacy events | `assets/repo-portal.js` + the repo data/intro/Station/Atelier blocks in `index.html` + `scripts/smoke.mjs`; preserve the existing plaza spawn for strict `?view=plaza&lang=en`, one repo request before Aha, `trafficKnown:false` for public API data, 15-minute/512 KiB LRU bounds, canonical `?repo=owner/repo&ref=repo-portal`, strict covered English-first `?repo=owner/repo&view=atelier&lang=en`, and [`docs/repo-portal-change-guide.md`](docs/repo-portal-change-guide.md). |
