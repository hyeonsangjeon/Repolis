...
| **`cloudflare-taxi/src/repository-atelier-grounding.js`** | MCP-first identity proof, exact anonymous public identity completion when needed, complete bounded README/license reads, and existing Entra-authenticated synthesis. | Changing Atelier document answers; preserve rejected/private-reference refusal, no redirects, the shared 25-second Worker deadline, and unchanged credentials/models/budgets. |
